    ...
    return (
        <main className="absolute flex flex-row justify-between w-full h-16 justifiy-center text-white p-2">
          ...
          <button className="flex flex-row my-auto mx-8" onClick={languageClick}><Image className="mx-2 my-auto" src={usa} alt='english' width={25} height={25}></Image>
            ...
            <div className={`${languagearrow ? 'absolute mt-12 flex flex-col w-fit insidebox h-fit rounded-md p-1 px-3 border-[1px] border-zinc-700 gap-3' : 'hidden'}`}>
            ...
            <button onClick={loginClickHandle} className="flex flex-row-reverse mr-4 px-6 pr-8 rounded-2xl redaccent h-7 text-sm my-auto"><div className="flex flex-row my-auto"><Image className="mx-1" src={steam} alt='steam' width={20} height={20}></Image>LOGIN</div></button>
            </div>
        </main> 
      );
}
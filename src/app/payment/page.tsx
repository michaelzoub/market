...
import { paymentOptions, cryptoOptions, cryptoSolo } from "../data/paymentdata"
            ...
            <div className={`${userIsLogged ? "visible w-full h-screen" : "hidden"}`}>
                <div className="mt-20 w-full">
                    ...
                    <div className="grid gap-2 items-grid flex flex-row">
                        {steamOption.map((e) => 
                        <Link href={`/payment/${e.slug}`} prefetch={true} className="flex flex-col searchbg items-center gap-2 p-1 rounded-sm hover:hoversearchbg">
                ...
                <div className="w-full mb-8">
                    ...
                    <div className="grid gap-2 items-grid flex flex-row">
                        {cryptoSolo.map((e) => 
                        <button className="flex flex-col searchbg items-center gap-2 p-1 rounded-sm hover:hoversearchbg">
                            <Image className="rounded-sm tradebox px-6 py-2 w-full" src={e.img} width={100} height={60} alt={e.name}></Image>
                            <div className="text-sm my-2 ">{e.name}</div>
                        </button>
                        )}
